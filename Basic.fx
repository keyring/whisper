...
float4 PS( VertexOut pin,
		   ...
		   uniform bool g_reflectionEnable) : SV_Target{
	...
	*/
	...
	if(g_lightCount > 0){
		...
		for(int i = 0; i < g_lightCount; ++i){
			...
			ComputeDirectLight(g_material, g_directLight[i], pin.NormalW, toEye, A, D, S);
...
technique11 Light1{
	...
	pass p0{
		SetVertexShader(ComileShader(vs_5_0, VS()));
		SetGeometryShader(NULL);
		SetPixelShader(CompiledShader(ps_5_0, PS(1, false, false)));
	}
}
